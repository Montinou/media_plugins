---
name: flow-assets
description: Use when generating game assets with Google Labs Flow — sprites, character turnarounds, map layers, dioramas — or when working with Flow applets, recipes, appletId, labs.google tools, or the aerthos-flow MCP tools
---
...
Las tools `flow_*` del MCP `aerthos-flow` cubren todo el ciclo. Este documento
...
Los dos pasos del medio son los que se saltean y los que causan los fallos:
**los valores de dropdown salen de `constants.ts`, no de adivinar**, y los
labels salen de `flow_inspect_controls`, no del código.
...
- `references/recipes.md` — esquema de recetas, `matrix`, vocabularios de Aerthos
- `references/api-map.md` — endpoints, auth de dos saltos, qué protege reCAPTCHA
- `references/applets.md` — catálogo de los applets de Aerthos y qué produce cada uno
